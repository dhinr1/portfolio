...
export default function DueDatePlanner() {
  ...
  const getPersonalDates = (dueDate) => {
    ...
    courseData.forEach(course => {
      const courseDate = new Date(firstDayOfLastMenstruation);
      courseDate.setDate(firstDayOfLastMenstruation.getDate() + (course.from_weeks_pregnant * 7));
      personalizedDates.push({
        course: course.course,
        personalizedDate: courseDate.toLocaleDateString("en-GB")
      });
    });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const dueDate = new Date(Number(selectedYear), Number(selectedMonth) - 1, Number(selectedDay));
    const dates = getPersonalDates(dueDate);
    setPersonalizedDates(dates);
  };
  ...
  const handleDownload = () => {
    ...
    personalizedDates.forEach((item, i) => {
  ...
  return (
    <div className="card-container">
      <div className="card-header">Get personalized course dates:</div>
      <div className="card-body">
        <h5 className="card-title">Your due date:</h5>
        <form onSubmit={handleSubmit} className="form-container">
          ...
          <button type="submit" className="card-button">Get dates</button>
        </form>

        {personalizedDates.length > 0 && (
          <div className={`course-plan ${personalizedDates.length > 0 ? "visible" : ""}`}>
            <div className="header-row">
              <p className="column-tag">Course name:</p>
              <p className="column-tag">From:</p>
            </div>
            <div className="all-courses">
              {personalizedDates.map((item, index) => (
                <p key={index}>
                  <span>{item.course}</span>
                  <span>{item.personalizedDate}</span>
                </p>
              ))}
            </div>
